...
use cw721::{ContractInfoResponse, Cw721Execute, Cw721ReceiveMsg, Expiration, Investor};
    ...
    pub fn execute(
        ...
        match msg {
            ExecuteMsg::Mint {
                ...
                value,
                nft_price,
            } => self.mint(deps, info, token_id, owner, token_uri, extension, value, nft_price),
            ExecuteMsg::BuyNFTs {
                token_id,
                counts,
            } => self.buy_nfts(deps, info, token_id, counts),
            ExecuteMsg::Approve {
                spender,
                token_id,
                expires,
    ...
    Q: CustomMsg,
{
    ...
    pub fn buy_nfts(
        ...
        let sent_token = &info.funds[0];

        if sent_token.amount < (token.nft_price * counts).into() {
            return Err(ContractError::Insufficient {});
        }

        let found_item = token.investors.iter_mut().find(|item| item.address == info.sender);
    ...
    pub fn set_deposit_denom(
        ...
        self.deposit_denom.save(deps.storage, &denom)?;
        Ok(Response::new()
            .add_attribute("action", "set_deposit_denom"))
    }
